<template>
  <div>
    <Menu>
      <li
        v-for="link in advocate.attributes.nav"
        :key="link.goTo"
      >
    ...
    >
      <section class="headlines">
        <h1>{{ advocate.attributes.title }}</h1>
        <p>{{ advocate.attributes.tagline }}</p>
        <p>{{ advocate.attributes.dates }}<br></p>
      </section>
...
function getBackgroundUris(background: string): [string, string] {
  const bgRoute = '/images/advocates/headers/'
...
@Component({
  ...
  async asyncData(context: Context) {
    ...
    const definition = await import(`~/src/advocates/${sourceName}.md`)
